...
title: "Parser"
icon: "filter"
description: "介绍 LangChain 输出解析器的作用、结构化输出关系和常见工程风险"
---
...
<Frame caption="输出解析器类继承关系">
  <img src="https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1761401570754-68908952-d1d3-4849-b04f-3b79bc4226f1.png" alt="输出解析器类继承关系" />
</Frame>
...
`StrOutputParser`是LangChain中最简单的输出解析器，它可以简单地将任何输入转换为字符串。从结果中提取content字段转换为字符串输出。

```python title="字符串解析器" icon="python"
...
```python title="Json 解析器" icon="python"
...
```python title="使用get_format_instructions 生成格式说明:" icon="python"
...
利用`CommaSeparatedListOutputParser`解析器，可以将模型的文本响应转换为一个用逗号分隔的列表(List\[str])

```python title="列表解析器" icon="python"
...
```python title="XML 解析器" icon="python"
...
```python title="Pydantic解析器" icon="python"
...
```python title="自定义输出解析器" icon="python"
import re
from langchain_core.output_parsers import PydanticOutputParser
from langchain_core.prompts import ChatPromptTemplate
from langchain_ollama import ChatOllama
